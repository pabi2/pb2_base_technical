...
class JasperClient:
    ...
    def run_report(self, report_part, report_format="pdf", params={}, exc_async=False):
        ...
        if r.status_code == 200:
            ...
            logging.info("JasperClient::Download Report Return: %s %s" % (r.status_code, r.content))
            raise osv.except_osv(_('Error !'), "JasperClient::Download Report Return Status:%s, Content:%s" % (r.status_code, r.content))
        ...
        if exportId and requestId:
            ...
            logging.info("JasperClient::Download Report From URL: %s" % (url))
            ...
            logging.info("JasperClient::Download Report Return exportId:%s, requestId:%s" % (exportId, requestId))
            return False
...
#         return res
    ...
    def search_ds_name(self, ds_uri_name):
        # ================= Search datasource
        url = '%s' % (
            self.get_url_resources()
        )
        params = {'folderUri': ds_uri_name, 'type': 'jdbcDataSource'}
        r = self.session.get(url=url, params=params)
        logging.info("%s" % (url))
        ...
        try:
            ...
            logging.info("%s" % (datasources_list))
            return datasources_list
        except:
            return False
        
    def delete_resources(self, res_uri):
        # ================= Search datasource
        url = '%s%s' % (
            self.get_url_resources(),
            res_uri
        )
        logging.info("JasperClient::Delete Resources: %s" % url)
    ...
    def delete_users(self, user_id):
        ...
        logging.info("JasperClient::Delete User: %s" % url)
        r = self.session.delete(url=url)
    ...
    def export_service(self, ds_uri, res_uri, user_id, filename):
        ...
        logging.info("JasperClient::Export ds_uri: %s" % ds_uri)
        logging.info("JasperClient::Export res_uri: %s" % res_uri)
        
        logging.info("JasperClient::Export")
    ...
    def import_service(self, filename):
        ...
        filename_import = 'import_' + filename + '.zip'
        
        headers = {
            "Content-Disposition": 'form-data; name="file"; filename="%s"' % (filename_import),
            "Content-Type": "application/zip",
            "X-Remote=Domain": "true"
        }
        dir_file = self.temp_path + filename_import
    ...
    # replace_serv_prd = "=o_nstda_or_th"
    # replace_serv_test = "o_test_28014_intra_nstda_or_th"
    # prd_db_url = "o-db.intra.nstda.or.th"
    # prd_db_port = "29001"
    # prd_db_dbname = "odoo"
    # test_db_url = "o-test-db.intra.nstda.or.th"
    # test_db_port = "29014"
    # test_db_dbname = "test"
    def modify_zipfile(self, filename, replace_serv_prd, replace_serv_test, prd_db_url, prd_db_port, prd_db_dbname, test_db_url, test_db_port, test_db_dbname):
        ...
        logging.info("JasperClient::modify_zipfile filename=%s [export_o_test_280xx.zip]" % (filename))
        logging.info("JasperClient::modify_zipfile replace_serv_prd=%s [o_nstda_or_th]" % (replace_serv_prd))
        logging.info("JasperClient::modify_zipfile replace_serv_test=%s [o_test_28014_intra_nstda_or_th]" % (replace_serv_test))
        logging.info("JasperClient::modify_zipfile prd_db_url=%s [o-db.intra.nstda.or.th]" % (prd_db_url))
        logging.info("JasperClient::modify_zipfile prd_db_port=%s [29001]" % (prd_db_port))
        logging.info("JasperClient::modify_zipfile prd_db_dbname=%s [odoo]" % (prd_db_dbname))
        logging.info("JasperClient::modify_zipfile test_db_url=%s [o-test-db.intra.nstda.or.th]" % (test_db_url))
        logging.info("JasperClient::modify_zipfile test_db_port=%s [29014]" % (test_db_port))
        logging.info("JasperClient::modify_zipfile test_db_dbname=%s [test]" % (test_db_dbname))
        
        conn_url_prd = "%s:%s/%s" % (prd_db_url, prd_db_port, prd_db_dbname)
        conn_url_test = "%s:%s/%s" % (test_db_url, test_db_port, test_db_dbname)
        
        dir_file = self.temp_path + filename
        dir_file_new = dir_file.replace('export_', 'import_')
    ...
    # Execute-only: 32
    # part_res = ''
    def modify_permissions_multi(self, role_id, role_part_res, role_recipient, role_mask, user_id, user_part_res, user_recipient, user_mask):
        url = '%s%s' % (
        ...
        data = """
          ...
          """ % (role_part_res, role_recipient, role_id, role_mask, user_part_res, user_recipient, user_id, user_mask)
        r = self.session.put(url=url, data=data, headers=headers)
        logging.info("JasperClient::put url:%s, data:%s, headers:%s" % (url, data, headers))
    ...
    def modify_permissions(self, user_id, part_res, recipient='user', mask=1):
        url = '%s%s' % (
            self.get_url_permissions(),
            part_res
        )
        ...
        data = """
          ...
          """ % (part_res, recipient,user_id, mask)
        r = self.session.put(url=url, data=data, headers=headers)
        logging.info("JasperClient::put url:%s, data:%s, headers:%s" % (url, data, headers))
        if r.status_code == 400:
            self._log(False, r.status_code, "A recipient or mask is invalid.")
            ...
            logging.info("JasperClient::modify_permissions uri:%s, recipient:%s:/%s, mask:%s" % (part_res, recipient, user_id, mask))
            self._log(True, r.status_code, "The request was successful.")
    ...
    def delete_user(self, user_id):
        if self.search_user(user_id):
            ...
            if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                self._log(True, r.status_code, "Delete User: %s Pass." % user_id)
                return True
            else:
                return False
        else:
    ...
    # ds_uri = API/Odoo/o.nstda.or.th/nstda_gnc
    # ds_name = CONN_NSTDA_GNC
    # conn_url = o.nstda
    # conn_port = 29001
    # conn_db = odoo
    # username = odoo
    ...
    def create_datasource(self, ds_uri, ds_name, conn_url, conn_port, conn_db, username, password):
        ...
        datasource = self.search_ds_name("/Datasources/%s" % (ds_uri))
        if not datasource and ds_name not in datasource:
